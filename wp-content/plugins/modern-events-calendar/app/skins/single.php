...
class MEC_skin_single extends MEC_skins
{
    ...
    public function display_next_previous_events($event)
    {
        ...
        {
            echo '<li class="mec-previous-event"><a class="mec-color mec-bg-color-hover mec-border-color" href="'.$this->main->get_event_date_permalink(get_permalink($p['post_id']), date('Y-m-d', $p['tstart'])).'"><i class="mec-fa-long-arrow-left"></i>'. __('PRV Event', 'mec') .'</a></li>';
        }

        if(is_array($n) and isset($n['post_id']))
        {
            echo '<li class="mec-next-event"><a class="mec-color mec-bg-color-hover mec-border-color" href="'.$this->main->get_event_date_permalink(get_permalink($n['post_id']), date('Y-m-d', $n['tstart'])).'">'. __('NXT Event', 'mec') .'<i class="mec-fa-long-arrow-right"></i></a></li>';
        }
    ...
    public function display_booking_widget($event, $event_m)
    {
        ...
        ?>
            <div class="mec-sold-tickets warning-msg"><?php _e('Sold out!', 'mec'); do_action('mec_booking_sold_out',$event, NULL, NULL, array($event->date)); ?></div>
