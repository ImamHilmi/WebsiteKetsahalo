...
class MEC_feature_books extends MEC_base
{
    ...
    public function register_post_type()
    {
        register_post_type($this->PT,
            array(
                ...
                'public'=>false,
                'show_ui'=>(current_user_can('mec_bookings') ? true : false),
                'show_in_menu'=>true,
                ...
                (
                    'read'=>'mec_bookings',
                    'read_post'=>'mec_bookings',
                    'read_private_posts'=>'mec_bookings',
                    'create_post'=>'mec_add_booking',
                    'create_posts'=>'mec_add_booking',
                    'edit_post'=>'mec_bookings',
                    'edit_posts'=>'mec_bookings',
                    'edit_private_posts'=>'mec_bookings',
                    'edit_published_posts'=>'mec_bookings',
                    'edit_others_posts'=>'mec_bookings',
                    'publish_posts'=>'mec_bookings',
                    'delete_post'=>'mec_bookings',
                    'delete_posts'=>'mec_bookings',
                    'delete_private_posts'=>'mec_bookings',
                    'delete_published_posts'=>'mec_bookings',
                    'delete_others_posts'=>'mec_bookings',
                ),
                'map_meta_cap'=>false
            )
        );
            ...
            <div id="mec_book_edit_form" class="mec-book-form mec-util-hidden" style="margin-top: 30px;">
                ...
                <div id="mec_book_edit_form_event_options">
                    ...
                    <div id="mec_date_tickets_booking_form_bfixed_fields">
                        <h3><?php _e('Booking Fields', 'mec'); ?></h3>
                        <hr>
    ...
    public function do_bulk_actions()
    {
        ...
        switch($action)
        {
            ...
            case 'ms-excel-export':
                ...
                $columns = array(__('ID', 'mec'), __('Event', 'mec'), __('Start Date & Time', 'mec'), __('End Date & Time', 'mec'), __('Order Time', 'mec'), $this->main->m('ticket', __('Ticket', 'mec')), __('Transaction ID', 'mec'), __('Total Price', 'mec'), __('Gateway', 'mec'), __('Name', 'mec'), __('Email', 'mec'), __('Ticket Variation', 'mec'), __('Confirmation', 'mec'), __('Verification', 'mec'));
                ...
                foreach($post_ids as $post_id)
                {
                    ...
                    foreach($attendees as $key => $attendee)
                    {
                        ...
                        $rendered_price = $this->main->render_price($this->book->get_ticket_total_price($transaction, $attendee, $post_id));

                        $ticket_id = isset($attendee['id']) ? $attendee['id'] : get_post_meta($post_id, 'mec_ticket_id', true);
                        $booking = array($post_id, get_the_title($event_id), date($datetime_format, $timestamps[0]), date($datetime_format, $timestamps[1]), $order_time, (isset($tickets[$ticket_id]['name']) ? $tickets[$ticket_id]['name'] : __('Unknown', 'mec')), $transaction_id, $rendered_price, $gateway_label, (isset($attendee['name']) ? $attendee['name'] : (isset($booker->first_name) ? trim($booker->first_name.' '.$booker->last_name) : '')), (isset($attendee['email']) ? $attendee['email'] : @$booker->user_email), trim($ticket_variations_output, ', '), $confirmed, $verified);
    ...
    public function save_book($post_id)
    {
        ...
        if($is_new_booking)
        {
            ...
            update_post_meta($post_id, 'mec_gateway_label', $gateway->title());
    ...
    public function book()
    {
        ...
        switch($step)
        {
            case '1':
                ...
                {
                    $wc = $this->getWC();
                    $next = $wc->cart($event_id, $date, $tickets)->next();
            ...
            case '2':
                ...
                {
                    $wc = $this->getWC();
                    $next = $wc->cart($event_id, $date, $tickets, $transaction_id)->next();
    ...
    public function bbf_date_tickets_booking_form()
    {
        ...
        {
            $output .= '<h3>'.__('Booking Fields', 'mec').'</h3>';
            foreach($bfixed_fields as $bfixed_field_id=>$bfixed_field)
            {
                ...
                {
                    $booking_bfixed_options .= '<input class="widefat" id="mec_book_bfixed_field_reg'.$bfixed_field_id.'" type="date" name="mec_fields['.$bfixed_field_id.']" value="" placeholder="'.__($bfixed_field['label'], 'mec').'" '.($mandatory ? 'required="required"' : '').' min="'.esc_attr(date('Y-m-d', strtotime('-100 years'))).'" max="'.esc_attr(date('Y-m-d', strtotime('+100 years'))).'" />';
                }
        ...
        if(count($reg_fields))
        {
            foreach($reg_fields as $reg_field_id=>$reg_field)
            {
                ...
                {
                    $booking_form_options .= '<input class="widefat" id="mec_book_reg_field_reg'.$reg_field_id.'" type="date" name="mec_attendee[reg]['.$reg_field_id.']" value="" placeholder="'.__($reg_field['label'], 'mec').'" '.($mandatory ? 'required="required"' : '').' min="'.esc_attr(date('Y-m-d', strtotime('-100 years'))).'" max="'.esc_attr(date('Y-m-d', strtotime('+100 years'))).'" />';
                }
    ...
    public function bbf_event_edit_options()
    {
        ...
        foreach($reg_fields as $reg_field_id=>$reg_field)
        {
            ...
            elseif($reg_field['type'] == 'date')
            {
                $booking_form_options .= '<input class="widefat" id="mec_book_reg_field_reg'.$key.'_'.$reg_field_id.'" type="date" name="mec_att['.$key.'][reg]['.$reg_field_id.']" value="" placeholder="'.__($reg_field['label'], 'mec').'" '.($mandatory ? 'required="required"' : '').' min="'.esc_attr(date('Y-m-d', strtotime('-100 years'))).'" max="'.esc_attr(date('Y-m-d', strtotime('+100 years'))).'" />';
            }
            elseif($reg_field['type'] == 'email')
            {
