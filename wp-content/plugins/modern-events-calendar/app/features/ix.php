...
class MEC_feature_ix extends MEC_base
{
    ...
    {
        add_submenu_page('mec-intro', __('MEC - Import / Export', 'mec'), __('Import / Export', 'mec'), 'mec_import_export', 'MEC-ix', array($this, 'ix'));
    }
    ...
    public function import_start_bookings()
    {
        ...
        if($name_end != 'csv') return array('success' => 0, 'message' => __('Please upload a CSV file.', 'mec'));

        // File Type is not valid
        if(!isset($feed_file['type']) or (isset($feed_file['type']) and !in_array(strtolower($feed_file['type']), array('text/csv')))) return array('success' => 0, 'message' => __('The file type should be CSV.', 'mec'));
        ...
        {
            unlink($target_path);
            return array('success' => 0, 'message' => __("Please upload a CSV file.", 'mec'));
        }
        ...
        return array('success' => (count($bookings) ? 1 : 0), 'message' => (count($bookings) ? __('The bookings are imported successfully!', 'mec') : __('No bookings found to import!', 'mec')));
    }
    ...
    public function import_start()
    {
        ...
        // File name validation
        $name_end = end(explode('.', $feed_file['name']));
        if(!in_array($name_end, array('xml', 'ics'))) return array('success' => 0, 'message' => __('Please upload an XML or an ICS file.', 'mec'));
        ...
        {
            unlink($target_path);
            return array('success' => 0, 'message' => __("Please upload an XML or an ICS file.", 'mec'));
        }
    ...
    public function export_all_events_do()
    {
        ...
        switch($format)
        {
            ...
            case 'csv':
                ...
                $columns = array(__('ID', 'mec'), __('Title', 'mec'), __('Start Date', 'mec'), __('Start Time', 'mec'), __('End Date', 'mec'), __('End Time', 'mec'), __('Link', 'mec'), $this->main->m('taxonomy_location', __('Location', 'mec')), __('Address', 'mec'), $this->main->m('taxonomy_organizer', __('Organizer', 'mec')), __('Organizer Tel', 'mec'), __('Organizer Email', 'mec'), $this->main->m('event_cost', __('Event Cost', 'mec')));
