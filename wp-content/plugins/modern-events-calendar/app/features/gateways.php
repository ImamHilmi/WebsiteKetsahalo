...
class MEC_gateway extends MEC_base implements MEC_gateway_interface
{
    ...
    public function validate($transaction_id)
    {
        ...
        foreach($tickets as $ticket_id => $quantity)
        {
            ...
            {
                if($availability[$ticket_id] == '0') $this->response(array('success' => 0, 'code' => 'SOLDOUT', 'message' => sprintf(__('%s ticket is sold out! Please go back and select another ticket if available.', 'mec'), $event_tickets[$ticket_id]['name'])));
                else $this->response(array('success' => 0, 'code' => 'SOLDOUT', 'message' => sprintf(__('Only %s slots remained for %s ticket so you cannot book %s ones.', 'mec'), $availability[$ticket_id], $event_tickets[$ticket_id]['name'], $quantity)));
            }
        }
    ...
    public function get_stripe_charge_description($transaction_id)
    {
        ...
        return sprintf(__('MEC Transaction ID: %s, Event Title: %s', 'mec'), $transaction_id, $event_title);
    }
}
...
class MEC_gateway_stripe extends MEC_gateway
{
    ...
    public function do_transaction($transaction_id = null)
    {
        ...
        update_post_meta($book_id, 'mec_gateway_label', $this->title());
    ...
    public function checkout_form_ideal($transaction_id, $options, $params = array())
    {
        ...
        <script type="text/javascript">
            ...
            jQuery('#mec_do_transaction_stripe_form<?php echo $transaction_id; ?>').on('submit', function(e)
            {
                ...
                var source = {
                    type: 'ideal',
                    amount: <?php echo (isset($transaction['price']) ? ((int) $this->stripe_multiply($transaction['price'], $this->main->get_currency_code())) : 0); ?>,
...
class MEC_gateway_stripe_connect extends MEC_gateway
{
    ...
    public function options_form()
    {
        ...
        ?>">
            ...
            <p class="mec-only-strip-connect-standard <?php echo ($connection_method == 'express' ? 'w-hide' : ''); ?>"><?php echo sprintf(__("You should set %s as Redirect URL in your Stripe dashboard.", 'mec'), '<code>'.rtrim(get_home_url(), '/').'/?mec-stripe-connect=1</code>'); ?></p>
            <br>
            <div class="mec-form-row">
                <label class="mec-col-3"
                       for="mec_gateways<?php echo $this->id(); ?>_title"><?php _e('Title', 'mec'); ?></label>
            ...
            <div class="mec-form-row">
                <label class="mec-col-3" for="mec_gateways<?php echo $this->id(); ?>_connection_method"><?php _e('Connection Method', 'mec'); ?></label>
                <div class="mec-col-9">
            ...
            <div class="mec-form-row">
                <div class="mec-col-12">
                    ...
                    <?php if($connection_method === 'express'): ?>
                        ...
                        <a id="mec_gateway_options_form_stripe_connection_button" class="button button-primary" onclick="mec_stripe_connection_checker();" href="<?php echo $this->main->URL('site'); ?>?mec-stripe-connect-express-redirect=1&_wpnonce=<?php echo wp_create_nonce('mec_stripe_connect_express_redirect'); ?>" target="_blank"><?php _e('Connect', 'mec'); ?></a>
                        <?php endif; ?>
                        ...
                        <div id="mec_gateway_options_form_stripe_connection_success" class="<?php echo $strip_account_id ? '' : 'mec-util-hidden'; ?>">
                            ...
                            <a href="<?php echo $this->main->URL('site'); ?>?mec-stripe-connect-express-dashboard=1&_wpnonce=<?php echo wp_create_nonce('mec_stripe_connect_express_dashboard'); ?>" target="_blank"><?php echo esc_html__('Visit your dashboard', 'mec'); ?></a>
                        </div>
                    <?php else: ?>
                        <?php if(!$strip_account_id): ?>
