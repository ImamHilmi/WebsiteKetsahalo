...
class MEC_main extends MEC_base
{
    ...
    public function get_sidebar_menu($active_menu = 'settings')
    {
        ...
        $settings = apply_filters('mec-settings-items-settings', array(
            ...
            __('User Events', 'mec') => 'user_events_options',
            ...
            __('AWeber Integration', 'mec') => 'aweber_option',
            __('MailPoet Integration', 'mec') => 'mailpoet_option',
            __('Sendfox Integration', 'mec') => 'sendfox_option',
        ), $active_menu);
        ...
        $single_event = apply_filters('mec-settings-item-single_event', array(
            ...
            __('Next / Previous Events', 'mec') => 'next_previous_events',
        ), $active_menu);
        ...
        $modules = apply_filters('mec-settings-item-modules', array(
            ...
            __('LearnDash Integration', 'mec') => 'learndash_options',
            __('PaidMembership Pro Integration', 'mec') => 'pmp_options',
        ), $active_menu);
        ...
        if($this->getPro())
        {
            $notifications_items = array_merge(array(
                __('Booking', 'mec') => 'booking_notification_section',
                __('Booking Confirmation', 'mec') => 'booking_confirmation',
                __('Booking Rejection', 'mec') => 'booking_rejection',
        ...
        if(get_query_var('verify'))
        {
            ...
            if($status == '1')
            {
                ...
                if(!$status_user)
                {
                    ...
                    echo '<p class="mec-success">'.__('Your booking has been verified successfully!', 'mec').'</p>';
                    return false;
                }
                else
                {
            ...
            $book = $this->getBook();
            if($book->verify($book_id)) echo '<p class="mec-success">'.__('Your booking has been verified successfully!', 'mec').'</p>';
        ...
        elseif(get_query_var('cancel'))
        {
            ...
            if($status == '-1')
            {
                ...
                if(!$status_user)
                {
                    ...
                    echo '<p class="mec-success">'.__('Your booking successfully canceled.', 'mec').'</p>';
                    return false;
                }
                else
                {
                    echo '<p class="mec-success">'.__('Your booking already canceled!', 'mec').'</p>';
                    return false;
                }
            }
            ...
            if($cancellation_period_from or $cancellation_period_to)
            {
                if($cancellation_period_from)
                {
                    ...
                    {
                        echo '<p class="mec-error">'.__("The cancelation window is not started yet.", 'mec').'</p>';
                        return false;
                    }
                }
    ...
    public function booking_invoice()
    {
        ...
        if(isset($_GET['method']) and sanitize_text_field($_GET['method']) == 'mec-invoice')
        {
            ...
            {
                wp_die(__("You don't have access to view this invoice!", 'mec'), __('Key is invalid.', 'mec'));
                exit;
            }
    ...
    */
            ...
            $messages = array(
                ...
                'others'=>array(
                    ...
                    'messages'=>array(
                        ...
                        'booking_button'=>array('label'=>__('Booking Button', 'mec'), 'default'=>__('Book Now', 'mec')),
    ...
    public function get_integrated_plugins_for_import()
    {
        return array(
            ...
            'wp-event-manager' => __('WP Event Manager', 'mec'),
        );
    }
    ...
    public function advanced_repeating_sort_day($start_of_week = 1, $day = 1)
    {
        ...
        array(__('Sun', 'mec'), __('Mon', 'mec'), __('Tue', 'mec'), __('Wed', 'mec'), __('Thu', 'mec'), __('Fri', 'mec'), __('Sat', 'mec'));
    ...
    public function get_flags($event, $date = NULL)
    {
        ...
        if(is_array($availability) and count($availability))
        {
            ...
            if(($total_bookings_limit > 0) and ($remained_tickets <= round((($percentage * $total_bookings_limit) / 100)))) return str_replace('%%title%%', __('Last Few Tickets', 'mec'), $output_tag);
    ...
    public function get_normal_labels($event, $display_label = false)
    {
        ...
        // Ongoing Event
        if($display_label and $this->is_ongoing($event)) $output .= '<span data-style="Normal" class="mec-label-normal mec-ongoing-normal-label">' . esc_html__('Ongoing', 'mec') . '</span>';
