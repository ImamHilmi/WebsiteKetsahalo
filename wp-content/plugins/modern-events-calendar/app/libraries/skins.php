...
class MEC_skins extends MEC_base
{
            ...
            elseif($type == 'date-range-picker')
            {
                ...
                $output .= '<div class="mec-date-search">
                    ...
                    <input class="mec-col-3 mec_date_picker_dynamic_format_start" data-min="'.$min_date.'" type="text"
                           ...
                           placeholder="'.esc_attr__('Start', 'mec').'" autocomplete="off">
                    <input class="mec-col-3 mec_date_picker_dynamic_format_end" type="text"
                           id="mec_sf_date_end_'.$this->id.'"
                           placeholder="'.esc_attr__('End', 'mec').'" autocomplete="off">
                </div>';
            }
        }
        elseif($field == 'time_filter')
        {
            if($type == 'local-time-picker')
            {
                ...
                $output .= '<div class="mec-time-picker-search">
                    ...
                    <input type="text" class="mec-timepicker-start" id="mec_sf_timepicker_start_'.$this->id.'" placeholder="'.__('Start Time', 'mec').'" data-format="'.$this->main->get_hour_format().'" />
                    <input type="text" class="mec-timepicker-end" id="mec_sf_timepicker_end_'.$this->id.'" placeholder="'.__('End Time', 'mec').'" data-format="'.$this->main->get_hour_format().'" />
                </div>';
            }
        }
        ...
        elseif($field == 'event_cost')
        {
            if($type == 'minmax')
            {
                $output .= '<div class="mec-minmax-event-cost">
                    ...
                    <input type="number" min="0" step="0.01" value="'.(isset($this->atts['event-cost-min']) ? $this->atts['event-cost-min'] : '').'" id="mec_sf_event_cost_min_'.$this->id.'" class="mec-minmax-price" placeholder="'.esc_attr__('Min Price', 'mec').'" />
                    <input type="number" min="0" step="0.01" value="'.(isset($this->atts['event-cost-max']) ? $this->atts['event-cost-max'] : '').'" id="mec_sf_event_cost_max_'.$this->id.'" class="mec-minmax-price" placeholder="'.esc_attr__('Max Price', 'mec').'" />
                </div>';
            }
        }
    ...
    public function booking_button($event, $type = 'button')
    {
        ...
        $title = $this->main->m('booking_button', __('Book Event', 'mec'));
    ...
    public function get_label_captions($event, $extra_class = null)
    {
        ...
        if(isset($event->data->labels) and is_array($event->data->labels) and count($event->data->labels))
        {
            foreach($event->data->labels as $label)
            {
                ...
                elseif($label['style'] == 'mec-label-canceled') $captions .= esc_html__($label['name'], 'mec');
                elseif($label['style'] == 'mec-label-custom' and isset($label['name']) and trim($label['name'])) $captions .= esc_html__($label['name'], 'mec');
                $captions .= '</span>';
